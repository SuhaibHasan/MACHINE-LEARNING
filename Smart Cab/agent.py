...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        for Qkey in self.Q[state]:

            if (maxQ == None) or (maxQ < self.Q[state][Qkey]):

                maxQ = self.Q[state][Qkey]
        return maxQ 
            ...
            if random.random() < self.epsilon:
                ...
                for key, value in self.Q[state].iteritems():
                    if value == self.get_maxQ(state):
                        actionValues.append(key)

                action = random.choice(actionValues)        
        return action
...
def run():
    ...
    agent = env.create_agent(LearningAgent, learning = True, alpha=.002, epsilon=1)
    ...
    sim = Simulator(env, display=False, update_delay=0.00, log_metrics=True, optimized=True)
